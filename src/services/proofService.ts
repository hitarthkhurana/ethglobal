...
export class ZKProofService {
  ...
  async generateProof(inputs: {
    ...
    const scaledLatitude = Math.round(
      inputs.sourceLocation[0] * SCALING_FACTOR
    );
    const scaledLongitude = Math.round(
      inputs.sourceLocation[1] * SCALING_FACTOR
    );
  ...
  async verifyProofOnChain(
    ...
    const formattedSignals = publicSignals.map((signal: string) =>
      BigInt(signal)
    );
